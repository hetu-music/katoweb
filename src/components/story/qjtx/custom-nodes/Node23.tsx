  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(3px 3px at 100px 50px, #ffffff, transparent), radial-gradient(4px 4px at 200px 150px, rgba(255,255,255,0.8), transparent), radial-gradient(2px 2px at 300px 250px, #ffffff, transparent), radial-gradient(5px 5px at 400px 350px, rgba(255,200,200,0.5), transparent), radial-gradient(3px 3px at 500px 100px, #ffffff, transparent)",
      ...
      >
        <div className="relative">
          <h2
            ...
            style={{
              color: titleColor,
              textShadow: `0 0 40px ${accentColor}, 0 2px 10px rgba(0,0,0,0.8)`,
            }}
          >
            {event.detail.title}
          </h2>
          <div
            className={`scrolly-glow-${event.id} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-1/2 bg-red-900/20 blur-3xl -z-10`}
          />
        </div>
      ...
      >
        ...
        >
          {event.detail.body.map((p, i) => (
            <p
              key={i}
              className={`scrolly-body-line my-4 ${p === "……" ? "opacity-30" : ""}`}
            >
              {p}
            </p>
          ))}
      ...
      {/* Closing */}
      <div
        className={`scrolly-closing-container-${event.id} absolute inset-0 flex items-center justify-center pointer-events-none`}
      >
        <div
          className={`scrolly-closing-${event.id} relative flex flex-col items-center`}
        >
...
export function animate(
  ...
  if (isMobile) {
    ...
    tl.fromTo(
      ...
      bodyLines,
      { opacity: 0, filter: "blur(15px)", duration: 2.5, ease: "power2.inOut" },
      "+=1.5",
    );
  }
  ...
  tl.set(bodyContainer, { display: "none" })
    ...
    .to(
      scrollyBg,
      { "--radius": "0px", duration: 4.0, ease: "power2.inOut" },
      "-=1.5",
    );
  ...
  if (snow) {
    gsap.to(snow, {
      backgroundPosition:
        "0px 600px, 0px 600px, 0px 600px, 0px 600px, 0px 600px",
      duration: 30,
      repeat: -1,
      ease: "none",
    });
