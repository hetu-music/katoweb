      ...
      <div className={`node-wrapper-${event.id} absolute inset-0`}>
        ...
        <div className={`scrolly-intro-${event.id} absolute inset-0 flex flex-col items-center justify-center px-4 md:px-8`}>
            <div className="relative flex flex-col items-center w-full max-w-[90vw]">
                <h2 className={`title-text-${event.id} relative text-4xl md:text-[5rem] text-transparent bg-clip-text bg-linear-to-b from-emerald-50 to-emerald-400/60 tracking-[0.6em] md:tracking-[0.8em] pl-[0.6em] md:pl-[0.8em] font-light text-center drop-shadow-[0_0_30px_rgba(16,185,129,0.4)] break-words leading-tight`}>
                    {detail.title}
                </h2>
                <div className={`title-line-${event.id} w-[1px] h-12 md:h-20 bg-linear-to-b from-emerald-400/60 to-transparent mt-8 md:mt-12`} />
            </div>
            
            <div className={`quote-text-${event.id} mt-6 md:mt-8 text-emerald-200/80 text-xs md:text-[15px] tracking-[0.6em] md:tracking-[0.8em] font-light text-center w-full max-w-[85vw] md:max-w-lg leading-[2.2] md:leading-[2.5] pl-[0.6em] md:pl-[0.8em] whitespace-normal break-words max-h-[40vh] overflow-y-auto no-scrollbar`}>
                {detail.quote}
            </div>
        </div>
        ...
        <div className={`scrolly-body-container-${event.id} absolute inset-0 flex items-center justify-center opacity-0 px-4 md:px-8`}>
            ...
            <div className={`astrolabe-${event.id} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120vw] h-[120vw] max-w-[1000px] max-h-[1000px] pointer-events-none flex items-center justify-center mix-blend-screen opacity-0`}>
                ...
                {/* Compass lines */}
                <div className="absolute top-[10%] bottom-[10%] left-1/2 -translate-x-1/2 w-[1px] bg-linear-to-b from-transparent via-emerald-400/15 to-transparent" />
                <div className="absolute left-[10%] right-[10%] top-1/2 -translate-y-1/2 h-[1px] bg-linear-to-r from-transparent via-emerald-400/15 to-transparent" />
                
                {/* Inner glowing core */}
                <div className="absolute w-[20%] h-[20%] rounded-full bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.08)_0%,transparent_70%)]" />
            </div>

            {/* Jade Slips (Fragments) - Fixed overflow by making it scrollable if too many items */}
            <div className="relative z-10 w-full max-w-5xl flex flex-wrap justify-center gap-4 md:gap-8 items-center content-center max-h-[85vh] overflow-y-auto no-scrollbar py-10 px-2">
                        ...
                        return (
                            <div key={`sep-${index}`} className={`jade-slip-${event.id} w-full flex justify-center py-2 opacity-0`}>
                                <div className="flex items-center gap-3 md:gap-5 opacity-60">
                                    <div className="h-[1px] w-12 md:w-20 bg-linear-to-r from-transparent to-emerald-500/40" />
                                    <div className="w-1.5 h-1.5 rotate-45 bg-emerald-400/80 shadow-[0_0_12px_rgba(52,211,153,0.8)]" />
                                    <div className="h-[1px] w-12 md:w-20 bg-linear-to-l from-transparent to-emerald-500/40" />
                                </div>
                            </div>
                        );
                    }
                    ...
                    return (
                        <div key={`item-${index}`} className={`jade-slip-${event.id} relative w-[85vw] md:w-[45%] lg:w-[30%] opacity-0`}>
                             <div className={`relative w-full h-full p-5 md:p-6 rounded-sm border ${isKeyLine ? 'border-emerald-400/30 bg-[#064e3b]/30 shadow-[0_0_25px_rgba(16,185,129,0.15)]' : 'border-emerald-600/10 bg-[#022c22]/20'} backdrop-blur-md overflow-hidden`}>
                                 ...
                                 {isKeyLine && (
                                     <>
                                         <div className="absolute top-0 left-0 w-8 h-[1px] bg-emerald-300/60" />
                                         <div className="absolute top-0 left-0 w-[1px] h-8 bg-emerald-300/60" />
                                         <div className="absolute bottom-0 right-0 w-8 h-[1px] bg-emerald-300/60" />
                                         <div className="absolute bottom-0 right-0 w-[1px] h-8 bg-emerald-300/60" />
                                         <div className="absolute inset-0 bg-emerald-500/5 blur-xl pointer-events-none" />
                                     </>
                                 )}

                                 <p className={`relative z-10 text-[13px] md:text-[15px] leading-[2.2] tracking-[0.15em] md:tracking-[0.2em] whitespace-normal break-words ${isKeyLine ? 'text-emerald-50 font-normal drop-shadow-[0_0_10px_rgba(52,211,153,0.5)]' : 'text-emerald-100/60 font-light'}`}>
                                     {paragraph}
                                 </p>
                             </div>
                        </div>
        ...
        <div className={`scrolly-closing-container-${event.id} absolute inset-0 flex flex-col items-center justify-center opacity-0 px-4 md:px-8`}>
            <div className={`closing-seal-${event.id} w-[1px] h-16 md:h-24 bg-linear-to-b from-transparent to-emerald-500/50 mb-8 md:mb-12`} />
            <div className={`closing-text-${event.id} text-emerald-200/90 text-sm md:text-lg tracking-[0.8em] md:tracking-[1em] pl-[0.8em] md:pl-[1em] font-light drop-shadow-[0_0_20px_rgba(16,185,129,0.6)] text-center w-full max-w-[90vw] leading-loose break-words whitespace-normal max-h-[60vh] overflow-y-auto no-scrollbar`}>
                {detail.closing}
            </div>
            <div className={`closing-seal-${event.id} w-[1px] h-16 md:h-24 bg-linear-to-t from-transparent to-emerald-500/50 mt-8 md:mt-12`} />
        </div>
...
export function animate(
  ...
  // Initial States
  tl.set([dust, bodyContainer, astrolabe, closingContainer], { opacity: 0 });
  ...
  // Exit Body
  .to(jadeSlips, { opacity: 0, y: -30, scale: 1.05, filter: "blur(15px)", duration: 3, stagger: 0.1, ease: "power2.in" }, "+=5")
  .to(astrolabe, { opacity: 0, scale: 1.2, duration: 3, ease: "power2.in" }, "-=2")
