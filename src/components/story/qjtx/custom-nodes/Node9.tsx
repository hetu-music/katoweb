      ...
      <div className={`node-wrapper-${event.id} absolute inset-0 opacity-0`}>
        ...
        <div className={`scrolly-intro-${event.id} absolute inset-0 flex flex-col items-center justify-center px-4 md:px-8`}>
            <div className="relative flex flex-col items-center w-full max-w-[90vw]">
                <div className="absolute -inset-10 bg-amber-100/50 blur-[40px] rounded-full pointer-events-none" />
                <h2 className="relative text-4xl md:text-[5rem] text-transparent bg-clip-text bg-linear-to-b from-rose-800 to-rose-600 tracking-[0.5em] md:tracking-[0.6em] pl-[0.5em] md:pl-[0.6em] font-medium text-center drop-shadow-[0_2px_10px_rgba(159,18,57,0.2)] break-words leading-tight">
                    {detail.title}
                </h2>
                <div className="w-1.5 h-1.5 rotate-45 bg-amber-400 mt-8 md:mt-12 shadow-[0_0_10px_rgba(251,191,36,0.8)]" />
            </div>
            
            <div className={`quote-text-${event.id} mt-6 md:mt-8 text-stone-600 text-xs md:text-[15px] tracking-[0.6em] md:tracking-[0.8em] font-light text-center w-full max-w-[85vw] md:max-w-lg leading-[2.2] md:leading-loose pl-[0.6em] md:pl-[0.8em] whitespace-normal break-words`}>
                {detail.quote}
            </div>
        </div>
        ...
        <div className={`scrolly-phase1-${event.id} absolute inset-0 flex flex-col items-center justify-center opacity-0 px-4 md:px-8`}>
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] h-[80vw] max-w-[600px] max-h-[600px] border-[1px] border-amber-300/30 rounded-full pointer-events-none" />
            <div className="relative z-10 flex flex-col items-center gap-8 md:gap-12 text-center max-w-3xl">
                {phase1Lines.map((line, i) => (
                    <p key={i} className="text-[15px] md:text-[18px] tracking-[0.25em] md:tracking-[0.35em] leading-[2.2] md:leading-[2.5] whitespace-normal break-words text-stone-700 font-light">
                        {line}
                    </p>
                ))}
            </div>
        ...
        <div className={`scrolly-phase2-${event.id} absolute inset-0 flex flex-col items-center justify-center opacity-0 px-4 md:px-8`}>
            ...
            <div className="relative z-10 flex flex-col items-center gap-6 md:gap-10 text-center max-w-4xl">
                {phase2Lines.map((line, i) => {
                    ...
                    return (
                        <p key={i} className={`text-[14px] md:text-[16px] tracking-[0.2em] md:tracking-[0.3em] leading-[2.2] md:leading-[2.5] whitespace-normal break-words ${
                            isSeparator ? "text-stone-400 tracking-[1em]" 
        ...
        <div className={`scrolly-phase3-${event.id} absolute inset-0 flex flex-col items-center justify-center opacity-0 px-4 md:px-8`}>
            ...
            <div className="relative z-10 flex flex-col items-center gap-6 md:gap-8 text-center max-w-3xl">
                {phase3Lines.map((line, i) => (
                    <p key={i} className="text-[16px] md:text-[22px] tracking-[0.3em] md:tracking-[0.4em] leading-[2.2] md:leading-[2.5] text-rose-900 font-normal drop-shadow-[0_2px_10px_rgba(159,18,57,0.1)] whitespace-normal break-words">
                        {line}
                    </p>
                ))}
            </div>
        ...
        <div className={`scrolly-phase4-${event.id} absolute inset-0 flex flex-col items-center justify-center opacity-0 px-4 md:px-8`}>
            <div className="relative z-10 flex flex-col items-center gap-8 md:gap-12 text-center max-w-4xl">
                {phase4Lines.map((line, i) => (
                    <p key={i} className={`text-[15px] md:text-[18px] tracking-[0.25em] md:tracking-[0.35em] leading-[2.2] md:leading-[2.5] whitespace-normal break-words ${line.includes("争寻画卷") ? "text-amber-700 font-normal drop-shadow-[0_0_10px_rgba(251,191,36,0.3)]" : "text-stone-700 font-light"}`}>
                        {line}
                    </p>
                ))}
            </div>
        ...
        <div className={`scrolly-closing-container-${event.id} absolute inset-0 flex flex-col items-center justify-center opacity-0 px-4 md:px-8`}>
            <div className="w-[1px] h-12 md:h-24 bg-linear-to-b from-transparent to-amber-500/50 mb-6 md:mb-10" />
            <div className="text-stone-500 text-xs md:text-base tracking-[0.8em] md:tracking-[1.2em] pl-[0.8em] md:pl-[1.2em] font-light text-center w-full max-w-[90vw] leading-loose break-words whitespace-normal">
                {detail.closing}
            </div>
            <div className="w-[1px] h-12 md:h-24 bg-linear-to-t from-transparent to-amber-500/50 mt-6 md:mt-10" />
        </div>
