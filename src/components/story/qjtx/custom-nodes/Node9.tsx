...
export function NodeLayout({
  ...
  return (
    ...
    >
      <div
        className={`node-wrapper-${event.id} absolute inset-0 bg-[#050202] opacity-0 pointer-events-none`}
      >
        ...
        {/* Floating embers */}
        <div
          className={`ambient-embers-${event.id} absolute inset-0 pointer-events-none opacity-0`}
        >
          {Array.from({ length: 20 }).map((_, i) => (
            <div
              key={i}
              className="absolute bg-rose-200/50 rounded-full blur-[1px]"
              style={{
        ...
        <div
          className={`ambient-petals-${event.id} absolute inset-0 pointer-events-none opacity-0 overflow-hidden`}
        >
          {Array.from({ length: 60 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`petal-${event.id} absolute ${color} ${blur} opacity-40 shadow-[0_0_10px_rgba(251,207,232,0.2)]`}
                style={{
        ...
        <div className="absolute inset-0 flex items-center justify-center p-4 md:p-8 z-10">
          {/* Intro Phase */}
          <div
            className={`content-intro-${event.id} absolute flex flex-col items-center text-center opacity-0`}
          >
          ...
          {/* Phase 1: Moli's Rise */}
          <div
            className={`content-phase1-${event.id} absolute flex flex-col gap-8 md:gap-10 items-center text-center w-full max-w-4xl opacity-0`}
          >
            {phase1Lines.map((l, i) => (
              <p
                key={i}
                className="text-stone-100 tracking-[0.2em] md:tracking-[0.3em] text-[16px] md:text-[20px] leading-loose drop-shadow-[0_4px_12px_rgba(0,0,0,0.9)] font-light"
              >
                {l}
              </p>
            ))}
          ...
          {/* Phase 2: The Fall */}
          <div
            className={`content-phase2-${event.id} absolute flex flex-col gap-8 md:gap-10 items-center text-center w-full max-w-3xl opacity-0`}
          >
            {phase2Lines.map((l, i) => (
              <p
                key={i}
                className={`tracking-[0.2em] md:tracking-[0.3em] text-[16px] md:text-[20px] leading-loose drop-shadow-[0_4px_12px_rgba(0,0,0,0.9)] font-light ${l === "……" ? "text-stone-400 tracking-[1em]" : "text-stone-300"}`}
              >
                {l}
              </p>
            ))}
          ...
          <div
            className={`content-phase3-${event.id} absolute flex flex-col gap-8 md:gap-10 items-center text-center w-full max-w-4xl opacity-0`}
          >
            {phase3Lines.map((l, i) => (
              <p
                key={i}
          ...
          <div
            className={`content-cinnabar-${event.id} absolute flex flex-col items-center justify-center w-full max-w-5xl opacity-0`}
          >
          ...
          {/* Phase 5: Echoes */}
          <div
            className={`content-phase5-${event.id} absolute flex flex-col gap-8 md:gap-10 items-center text-center w-full max-w-4xl opacity-0`}
          >
            {phase4Lines.map((l, i) => (
              <p
                key={i}
                className={`tracking-[0.25em] md:tracking-[0.35em] text-[16px] md:text-[20px] leading-loose drop-shadow-[0_4px_12px_rgba(0,0,0,0.9)] font-light ${l.includes("争寻画卷") ? "text-rose-200" : "text-stone-200"}`}
              >
                {l}
              </p>
            ))}
          ...
          {/* Closing */}
          <div
            className={`content-closing-${event.id} absolute flex flex-col items-center justify-center opacity-0`}
          >
            <div className="w-px h-16 md:h-24 bg-stone-300/50 mb-8 md:mb-12" />
            <div className="text-stone-300 tracking-[0.8em] md:tracking-[1.2em] text-xs md:text-base pl-[0.8em] md:pl-[1.2em] max-w-[85vw] whitespace-normal wrap-break-word text-center font-light drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]">
              {detail.closing}
            </div>
            <div className="w-px h-16 md:h-24 bg-stone-300/50 mt-8 md:mb-12" />
          </div>
        </div>
      </div>
    </div>
...
export function animate(
  ...
  if (bgImg) {
    ...
    tl.to(
      bgImg,
      { filter: "grayscale(0%)", duration: 6, ease: "power2.out" },
      "-=0.5",
    );
  }
  tl.fromTo(
