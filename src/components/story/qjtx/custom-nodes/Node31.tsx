    ...
    <div className={`scrolly-text-${event.id} relative z-10 h-full w-full overflow-hidden font-serif select-none`}>
      ...
      <div className={`node-wrapper-${event.id} absolute inset-0 opacity-0`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className={`silver-thread-${event.id} w-[1px] h-[80vh] bg-linear-to-b from-transparent via-slate-400/30 to-transparent opacity-0`} />
        </div>
        ...
        <div className={`content-phase-0-${event.id} absolute inset-0 flex flex-col items-center justify-center px-4 md:px-8 pointer-events-none`}>
            ...
            <h2 className="relative text-4xl md:text-[5rem] text-transparent bg-clip-text bg-linear-to-b from-slate-50 to-slate-400/80 tracking-[0.5em] md:tracking-[0.6em] pl-[0.5em] md:pl-[0.6em] font-light text-center drop-shadow-[0_0_20px_rgba(241,245,249,0.3)] break-words leading-tight max-w-[90vw]">
                {detail.title}
            </h2>
            <div className="mt-10 text-slate-300/80 text-xs md:text-[15px] tracking-[0.4em] md:tracking-[0.6em] font-light text-center w-full max-w-2xl leading-[2.2] md:leading-[2.5] pl-[0.4em] md:pl-[0.6em] whitespace-normal break-words drop-shadow-md">
                {detail.quote}
            </div>
        </div>
        ...
        <div className={`content-phase-1-${event.id} absolute inset-0 flex flex-col items-center justify-center px-6 md:px-12 pointer-events-none`}>
            <div className="flex flex-col gap-8 md:gap-12 max-w-3xl text-center">
                {phase1Lines.map((line, i) => (
                    <p key={i} className={`text-[14px] md:text-[16px] leading-[2.2] tracking-[0.2em] md:tracking-[0.3em] pl-[0.2em] md:pl-[0.3em] text-slate-200/90 font-light whitespace-normal break-words drop-shadow-sm`}>
                        {line}
                    </p>
                ))}
            </div>
        ...
        <div className={`content-phase-2-${event.id} absolute inset-0 flex flex-col items-center justify-center px-6 md:px-12 pointer-events-none`}>
            <div className="flex flex-col gap-8 md:gap-12 max-w-4xl text-center relative z-10">
                {phase2Lines.map((line, i) => {
                    ...
                    return (
                        <p key={i} className={`text-[14px] md:text-[16px] leading-[2.2] tracking-[0.2em] md:tracking-[0.3em] pl-[0.2em] md:pl-[0.3em] whitespace-normal break-words transition-colors duration-1000 ${
                            emphasis ? 'text-slate-50 font-normal drop-shadow-[0_0_15px_rgba(255,255,255,0.6)]' 
        ...
        <div className={`content-phase-3-${event.id} absolute inset-0 flex flex-col items-center justify-center px-6 pointer-events-none`}>
             <div className="w-[1px] h-16 md:h-24 bg-linear-to-b from-transparent to-slate-400/30 mb-10" />
             <div className="text-slate-100 text-sm md:text-xl tracking-[0.8em] md:tracking-[1.2em] pl-[0.8em] md:pl-[1.2em] font-light max-w-4xl text-center leading-[2.5] md:leading-[3] drop-shadow-[0_0_20px_rgba(241,245,249,0.5)] whitespace-normal break-words">
                 {detail.closing}
             </div>
             <div className="w-[1px] h-16 md:h-24 bg-linear-to-t from-transparent to-slate-400/30 mt-10" />
        </div>
