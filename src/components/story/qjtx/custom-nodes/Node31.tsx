    ...
    <div className={`scrolly-text-${event.id} relative z-10 h-full w-full overflow-hidden font-serif select-none`}>
      ...
      <div className={`node-wrapper-${event.id} absolute inset-0 opacity-0 bg-[#020617]`}>
        ...
        <div className={`content-intro-${event.id} absolute inset-0 flex flex-col items-center justify-center px-4 md:px-8 pointer-events-none z-10 opacity-0`}>
          ...
          <h2 className={`title-text-${event.id} text-5xl md:text-[6rem] tracking-[0.5em] md:tracking-[0.6em] pl-[0.5em] md:pl-[0.6em] font-light text-center wrap-break-word leading-tight max-w-[90vw]`}>
            {detail.title}
          </h2>
          <div className={`quote-text-${event.id} mt-8 md:mt-10 text-xs md:text-[16px] tracking-[0.6em] md:tracking-[0.8em] font-light text-center w-full max-w-[85vw] md:max-w-2xl leading-[2.5] pl-[0.6em] md:pl-[0.8em] whitespace-normal wrap-break-word`}>
            {detail.quote}
          </div>
            ...
            <div className="relative p-10 md:p-20 max-w-4xl bg-[radial-gradient(circle_at_50%_50%,rgba(2,6,23,0.6)_0%,transparent_80%)]">
              ...
              <div className="flex flex-col gap-6 md:gap-8 text-center">
                {lines.map((line, i) => {
                  ...
                  return (
                    <p key={i} className={`text-[15px] md:text-[18px] leading-[2.5] tracking-[0.3em] md:tracking-[0.4em] whitespace-normal wrap-break-word ${emphasis ? 'text-rose-100 drop-shadow-[0_0_15px_rgba(225,29,72,0.4)] font-normal' : 'text-slate-300 font-light'}`}>
                      {line}
                    </p>
                  );
                })}
