          ...
          <div className="relative w-full max-w-[1200px] h-[80vh] md:h-[75vh] pointer-events-auto flex items-center justify-center">
            ...
            >
              ...
              <div
                ...
                style={{
                  backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.12'/%3E%3C/svg%3E")`,
                }}
              />
              ...
              <div className="relative h-full w-max min-w-full flex flex-row-reverse items-center justify-center gap-12 md:gap-20 px-12 md:px-24 box-border z-10">
                {/* Title */}
                <div
                  className={`content-item-${event.id} content-intro-${event.id} [writing-mode:vertical-rl] text-4xl md:text-[4.5rem] text-[#2c1e16] tracking-[0.3em] font-medium shrink-0 max-h-[85%] drop-shadow-sm`}
                >
                  {detail.title}
                </div>
                ...
                {detail.quote && (
                  <div
                    className={`content-item-${event.id} content-intro-${event.id} [writing-mode:vertical-rl] text-sm md:text-[15px] text-[#5d4037] tracking-[0.4em] shrink-0 max-h-[85%]`}
                  >
                    {detail.quote}
                  </div>
                )}
                ...
                {detail.body.map((p, i) => (
                  <div
                    key={i}
                    className={`content-item-${event.id} content-body-${event.id} [writing-mode:vertical-rl] text-[14px] md:text-[16px] leading-[2.2] md:leading-[2.5] tracking-[0.25em] text-[#2c1e16] shrink-0 max-h-[75%] wrap-break-word ${p === "……" ? "opacity-50" : "font-light"}`}
                  >
                    {p}
                  </div>
                ))}
                ...
                {detail.closing && (
                  <div
                    className={`content-item-${event.id} content-closing-${event.id} [writing-mode:vertical-rl] text-xs md:text-sm tracking-[0.5em] text-[#8B1818] font-bold border-2 border-[#8B1818] px-3 py-4 md:px-4 md:py-6 bg-[#8B1818]/5 shrink-0 max-h-[85%] drop-shadow-[0_0_8px_rgba(139,24,24,0.15)]`}
                  >
                    {detail.closing}
                  </div>
                )}
            ...
            {/* Wooden Rollers */}
            <div
              className={`left-roller-${event.id} absolute left-1/2 top-[-2%] bottom-[-2%] w-5 md:w-8 rounded-full bg-linear-to-r from-[#1a110b] via-[#4a3525] to-[#1a110b] shadow-[10px_0_20px_rgba(0,0,0,0.6)] z-20 border-x border-[#5D4037]/30`}
            />
            <div
              className={`right-roller-${event.id} absolute left-1/2 top-[-2%] bottom-[-2%] w-5 md:w-8 rounded-full bg-linear-to-l from-[#1a110b] via-[#4a3525] to-[#1a110b] shadow-[-10px_0_20px_rgba(0,0,0,0.6)] z-20 border-x border-[#5D4037]/30`}
            />
          </div>
        </div>
      </div>
...
export function animate(
  ...
  const introItems = Array.from(selAll(`.content-intro-${eventId}`));
  const closingItems = Array.from(selAll(`.content-closing-${eventId}`));
  const bodyItems = Array.from(selAll(`.content-body-${eventId}`));
  const isMobile = window.innerWidth < 768;
  const bodyGroupSize = Math.ceil(bodyItems.length / 3);
  const bodyGroup1 = bodyItems.slice(0, bodyGroupSize);
  const bodyGroup2 = bodyItems.slice(bodyGroupSize, bodyGroupSize * 2);
  const bodyGroup3 = bodyItems.slice(bodyGroupSize * 2);
  ...
  if (isMobile) {
    ...
    tl.set([...bodyGroup1, ...bodyGroup2, ...bodyGroup3, ...closingItems], {
      display: "none",
      opacity: 0,
      filter: "blur(10px)",
    });
  ...
  const updateScroll = (progress: number) => {
    ...
    gsap.set(paperContainer, {
      clipPath: `inset(0 ${clipVal}% 0 ${clipVal}%)`,
    });
  ...
  if (isMobile) {
    tl.to(
      ...
      .set(introItems, { display: "none" })
      .set(bodyGroup1, { display: "", opacity: 0, filter: "blur(10px)" })
      .to(bodyGroup1, {
        opacity: 1,
        filter: "blur(0px)",
        duration: 2.0,
      ...
      .set(bodyGroup1, { display: "none" })
      .set(bodyGroup2, { display: "", opacity: 0, filter: "blur(10px)" })
      .to(bodyGroup2, {
        opacity: 1,
        filter: "blur(0px)",
        duration: 2.2,
      ...
      .set(bodyGroup2, { display: "none" })
      .set(bodyGroup3, { display: "", opacity: 0, filter: "blur(10px)" })
      .to(bodyGroup3, {
        opacity: 1,
        filter: "blur(0px)",
        duration: 2.0,
      ...
      .set(bodyGroup3, { display: "none" })
      .set(closingItems, { display: "", opacity: 0, filter: "blur(10px)" })
      .to(closingItems, {
        opacity: 1,
        filter: "blur(0px)",
        duration: 1.8,
    ...
    tl.to(
      ...
      .set(introItems, { display: "none" })
      .set(bodyItems, { display: "", opacity: 0, filter: "blur(10px)" })
      .to(bodyItems, {
        opacity: 1,
        filter: "blur(0px)",
        duration: 2.4,
